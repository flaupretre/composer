...
//        $prependAutoloader = $config->get('prepend-autoloader') === false ? 'false' : 'true';
        $prependAutoloader = 'false'; // Required for Automap
...
EOF;
        ...
        // flatten array
        if ($scanPsr0Packages) {
            // Scan the PSR-0/4 directories for PHP files, and add symbols to the map
        ...
        self::copyFile(__DIR__.'/automap.phk',$targetDir.'/automap.phk');
...
class ComposerAutoloaderInit$suffix
{
    ...
    // Load and register Automap map (if defined)
    private static function registerAutomapLoader()
    {
        \$mapPath=__DIR__.'/../../auto.map';
        if (file_exists(\$mapPath)) {
            if (!class_exists('Automap\\Map',false)) {
                // Work around PHP bug #39903 ('__COMPILER_HALT_OFFSET__ already defined')
                error_reporting((\$errlevel=error_reporting()) & ~E_NOTICE);
                require(__DIR__.'/automap.phk');
                error_reporting(\$errlevel);
            }
            \\Automap\\Mgr::load(\$mapPath);
        }
    }
...
INCLUDEPATH;
        ...
        // Automap must be called first
        $file .= <<<REGISTER_AUTOMAP
        self::registerAutomapLoader();
