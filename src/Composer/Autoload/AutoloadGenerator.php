...
require(__DIR__.'/Automap/automap.phk');    /* Load Automap builder */
...
// Composer is now using Automap for map-based autoloading.
// This file is here for BC only and always returns an empty array.
...
EOF;
            ...
            foreach ($namespacesToScan as $namespace => $groups) {
                foreach ($groups as $group) {
                    ...
                    foreach ($group['paths'] as $dir) {
                        ...
                        self::mapScan($map, $mapPath, $dir, $whitelist, $namespaceFilter);
                    }
                }
            }
        }
        ...
        foreach ($autoloads['classmap'] as $dir) {
            ...
            self::mapScan($map,$mapPath,$dir);
        }

        if ($map->symbolCount()) {
            $map->save($mapPath);
        } else {
            $filesystem->remove($mapPath);
        }
        ...
        file_put_contents($targetDir.'/autoload_real.php', $this->getAutoloadRealFile(($map->symbolCount() != 0), (bool) $includePathFile, $targetDirLoader, (bool) $includeFilesFile, $vendorPathCode, $appBaseDirCode, $suffix, $useGlobalIncludePath, $prependAutoloader, $classMapAuthoritative));

        self::copyFile(__DIR__.'/ClassLoader.php',$targetDir.'/ClassLoader.php');
        self::copyFile(__DIR__.'/Automap/Mgr.php',$targetDir.'/Mgr.php');
        self::copyFile(__DIR__.'/Automap/Map.php',$targetDir.'/Map.php');
...
/**
     ...
     *
     * @param \Automap_Creator $map The map we are addaing symbols to
     * @param string $mapPath The path of this map file
     * @param \Iterator|string $paths      The path to search in or an iterator
     * @param string           $whitelist Regex that matches against the file path
     * @param string           $namespace Optional namespace prefix to filter by
     *
     * @return null
     */
    public static function mapScan($map, $mapPath, $paths, $whitelist = null, $namespace = null)
    {
        if (is_string($paths)) $paths=array($paths);
        $mapPath=strtr($mapPath, '\\', '/');

        foreach ($paths as $path) {
            $rpath=self::getRelativePath($path,dirname($mapPath));
            $map->registerPath($path,$rpath,$namespace,$whitelist);
        }
    }
    
    private static function getRelativePath($path,$refpath)
    {
        $path=rtrim(strtr($path, '\\', '/'),'/');
        $refpath=rtrim(strtr($refpath, '\\', '/'),'/');
        if ($path===$refpath) return '.';
        if (strpos($path.'/',$refpath.'/')===0) return substr($path,strlen($refpath)+1);
        return $path;
    }

    private static function copyFile($spath,$tpath)
    {
...
PSR4;
        ...
        if ($useClassMap) {
            $file .= <<<'CLASSMAP'
        $loader->addClassMap(dirname(dirname(__DIR__)).'/auto.map');
CLASSMAP;
        }
